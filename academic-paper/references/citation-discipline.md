...
Use this reference when the built-in DBLP/CrossRef workflow in `academic-paper` Phase 5a is not enough.
...
> file); submission-time bibliography audit → `citation-audit` skill;
> numerical-claim audit → `claim-audit` skill.
...
fabricated arXiv IDs / DOIs / titles propagate into literature surveys,
landscape surveys, or downstream writing.

In environments with external script support, this protocol can be implemented
by a `verify_papers.py` helper. In this suite, the same function is performed
by the `citation-audit` skill using Tavily MCP for live verification, or by
manual cross-checking against DBLP/CrossRef/arXiv during the writing phase.
...
**Caching.** If using an automated helper script, default cache scope is
per-project at `<project>/.cache/verify_papers.json` with 30-day TTL.
Cache keys use canonical identifier priority:
`arxiv:{id_without_version}` → `doi:{lowercase}` →
`title:{sha1_of_normalized_title}[:16]`. The cache value preserves all known
identifiers so the same paper found through different search paths reconciles
correctly.

**CrossRef User-Agent.** When calling CrossRef API directly, set
`VERIFY_EMAIL` to your institutional email to reduce rate-limit risk.
A placeholder is fine for low-volume use, but CrossRef prefers a real contact
for higher polite-pool throughput.
...
or `claim-audit` (which verifies numerical claims against raw result
files). Run pre-search verification on candidate lists; run the audit skills
before submission.

**In this suite.** The `citation-audit` skill performs submission-time
verification using Tavily MCP for live lookup. For pre-search filtering
during the literature survey phase (`academic-paper` Phase 1), use Tavily
MCP directly to cross-check candidate papers, or perform manual verification
against the sources listed above. No external helper script is required.
