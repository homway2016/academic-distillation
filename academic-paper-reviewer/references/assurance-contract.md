...
Audits in this suite emit machine-readable verdicts. The `assurance` axis decides whether
those verdicts are advisory (draft mode) or load-bearing gates (submission mode).
This contract is referenced by `academic-paper`, `claim-audit`, `citation-audit`,
and `proof-checker` skills.
...
- Silent skip allowed.
- `academic-paper` Phase 6 produces a final report regardless.
...
- `academic-paper` Phase 6 checks that all mandatory audits have emitted a verdict;
  any `FAIL` or `BLOCKED` blocks Final Report.
- The Final Report tags itself `submission-ready: yes/no` based on audit output.
...
```json
{
  ...
  "trace_path": "rebuttal/traces/paper-claim-audit/2026-04-21_run01/",  // optional: where trace files are stored
  "session_id": "019dae73-fc12-4ab8-...",  // optional: session identifier for resume
  "reviewer_model": "kimi-2.6",  // optional: model used for internal review persona
